...
<body>
  ...
  <script>
    ...
    const NEGATORS = new Set(["not","no","never","dont","don't","didnt","didn't","isnt","isn't","wasnt","wasn't","cant","can't","wont","won't","cannot"]);
    ...
    function heuristicPredict(payload){
      ...
      let pos=0, neg=0, s_pos=0, s_neg=0;
      ...
      const len_bonus = 0.015 * Math.min(n, 260);
      const excl_bonus = 0.10 * Math.min(excl, 12);
      const quest_pen  = 0.08 * Math.min(quest, 10);

      const raw = (s_pos - 1.15*s_neg) + len_bonus + excl_bonus - quest_pen + vote_score;

      const centers = [-3.0, -1.4, 0.0, 1.6, 3.2]; // 1..5
      const temp = 1.35;
...
E[stars]=${expStar.toFixed(3)}  maxP=${Math.max(...probs).toFixed(3)}`;

      return { stars, probs, model: "heuristic_rulebase_v1", details };
    }
